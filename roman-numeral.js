...
function getNumberByPosition(number) {
    let numberSequence = number.toString().split('');          // [ '2', '1', '8' ]                              
    let multiplier = Math.pow(10, numberSequence.length - 1);  // 100                                 
    ...
    return numberSequence.map((elem) => {
        ...
        multiplier /= 10;                        
        return numberByPosition;              // { amount: '2', positionHouse: 100 }, { amount: '1', positionHouse: 10 }, { amount: '8', positionHouse: 1 }
    });
}
